...
WORKSPACE_ROOT = Path(
    os.getenv("VIDSYNTH_WORKSPACE_ROOT", Path(__file__).resolve().parents[1] / "workspace")
).expanduser().resolve()

# --- SETTINGS ---
SETTINGS = {
    # Path to the clips.json generated by Step 1
    "input_clips_json": str(WORKSPACE_ROOT / "segmentation" / "Summer_vacation" / "clips.json"),
    
    # Path to the source video file used in Step 1
    # NOTE: Update this to match your actual video file path if different
    "source_video_path": str(WORKSPACE_ROOT / "videos" / "Summer_vacation.mp4"),
    
    # Output directory for clustering results
    "output_dir": str(WORKSPACE_ROOT / "clusters" / "Summer_vacation"),
